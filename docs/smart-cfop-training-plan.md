...
| `11` | `EOCP` | `Tools.randomLastLayer()`，过滤 OLL 已完成状态 | EOLL + 顶层 CP 完成 |
...
- `EOCP` 生成复用随机 OLL 起点，即 F2L 完成、最后一层朝向未固定，并过滤 OLL 已完成状态；完成判断要求顶层棱朝向正常且四个顶层角块在正确位置，允许角朝向和顶层棱排列未完成，可作为 `2GLL` 起点。
